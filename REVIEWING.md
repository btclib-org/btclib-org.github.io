...
- **Does a change to the derivation keep the copy comparable?** What
  makes the generated file auditable is that its body is the source's
  bytes and nothing else, so a transformation added to
  `.github/scripts/derive-homepage.sh` — a rewritten link, a stripped
  heading — is a finding unless the script also refuses the shape it was
  added for. The rejected alternatives are in the script's own header.
...
- **Does a claim about what Pages serves come from `gh api`?** The build
  ...
  them that no call answers is the defect this tree is most able to hide.
